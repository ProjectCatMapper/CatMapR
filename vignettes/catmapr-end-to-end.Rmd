...
| --- | --- | --- |
| `/:database/explore` | Search categories/entities and inspect details | `searchDatabase()`, `CMIDinfo()` |
...
| Submit metadata/category edits | `/:database/edit` | Upload edit rows and refresh USES | `submitEditUpload(...)` or `uploadInputNodes(...)` + `updateWaitingUSES(...)` |
...
- `so = "standard"` expects full key expressions in the selected key column (for example `VARIABLE == VALUE`).
- `so = "simple"` expects raw values only (for example `eth:yoruba`) without `==`.

If preformatted key expressions are provided in `so = "simple"`, CatMapR warns
and strips the left-hand side before upload.
