...
CatMapR provides these capabilities in scriptable R workflows, including translation,
...
`get_properties()` is also available for deployments that expose
...
Upload calls always use standard key expressions.
Provide full key expressions in the selected key column
(for example `VARIABLE == VALUE`).

`upload_rows()` triggers contextual USES relationship refresh in the graph
database based on USES properties that connect to other CMIDs. This trigger is
fire-and-forget and is not polled for completion.
