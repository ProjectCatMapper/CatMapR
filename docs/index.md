...
Project page (package docs):
<https://projectcatmapper.github.io/CatMapR/>

This site contains the CatMapR package documentation, including function
reference pages, articles, and usage guidance.

![CatMapper logo](reference/figures/catmapper-logo.webp)![SocioMap
logo](reference/figures/sociomap-logo.webp)![ArchaMap
logo](reference/figures/archamap-logo.webp)

**CatMapR** is an R package that provides an interface to the [CatMapper
API](https://catmapper.org), facilitating access to dataset catalog
metadata, categories, and entities managed within CatMapper’s systems,
including `SocioMap` and `ArchaMap`. CatMapper organizes complex
category systems—such as ethnicities, languages, religions, political
districts, and artifacts—frequently used in social science and
archaeological research.

This package allows users to:

- Retrieve dataset catalog metadata from CatMapper databases.
- Search for specific categories or entities and obtain detailed
  information.
- Translate terms within datasets based on domain-specific categories,
  enabling data consistency and integration across diverse datasets.
- Discover domain and property metadata used by CatMapper deployments.
- Create and join datasets across different domains for integrated
  analysis.
- Submit authenticated edit/upload operations using API-key-based write
  access.

## Installation

Currently, **CatMapR** is only available on GitHub. To install it, use
...
- **[`listDatasetMetadata()`](https://projectcatmapper.github.io/CatMapR/reference/listDatasetMetadata.md)**:
  List dataset catalog metadata for `SocioMap` or `ArchaMap`.
- **[`getDatasetMetadata()`](https://projectcatmapper.github.io/CatMapR/reference/getDatasetMetadata.md)**:
  Retrieve metadata for a specific dataset CMID.
- **[`CMIDinfo()`](https://projectcatmapper.github.io/CatMapR/reference/CMIDinfo.md)**:
  Fetch details about a specific entity by CatMapper ID (CMID).
- **[`searchDatabase()`](https://projectcatmapper.github.io/CatMapR/reference/searchDatabase.md)**:
  Search for terms within a database, optionally filtering by domain,
  property, year, and context.
- **[`translate()`](https://projectcatmapper.github.io/CatMapR/reference/translate.md)**:
  Translate terms within data frames by matching specified properties
  and domains.
- **[`createLinkfile()`](https://projectcatmapper.github.io/CatMapR/reference/createLinkfile.md)**:
  Propose merge keys for selected datasets within a category domain.
- **[`joinDatasets()`](https://projectcatmapper.github.io/CatMapR/reference/joinDatasets.md)**:
  Join two aligned datasets through CatMapper matching infrastructure.
- **[`getDomains()`](https://projectcatmapper.github.io/CatMapR/reference/getDomains.md)**:
  Retrieve CatMapper domain/subdomain metadata.
- **[`getUploadProperties()`](https://projectcatmapper.github.io/CatMapR/reference/getUploadProperties.md)**:
  Retrieve upload-oriented property metadata grouped into node and USES
  relationship fields.
- **[`getProperties()`](https://projectcatmapper.github.io/CatMapR/reference/getProperties.md)**:
  Retrieve flattened property metadata from API deployments that expose
  `/metadata/properties/<database>`.
- **[`build_key()`](https://projectcatmapper.github.io/CatMapR/reference/build_key.md)**:
  Build upload-ready key expressions in `FIELD == VALUE` format.
- **[`normalize_key()`](https://projectcatmapper.github.io/CatMapR/reference/normalize_key.md)**:
  Normalize stored-form keys for reuse in upload workflows.
- **[`is_normalized_key()`](https://projectcatmapper.github.io/CatMapR/reference/is_normalized_key.md)**:
  Check whether key values are upload-ready expressions.
- **[`prepare_edit_upload()`](https://projectcatmapper.github.io/CatMapR/reference/prepare_edit_upload.md)**:
  Validate upload payload shape and mode before write calls.
- **[`uploadInputNodes()`](https://projectcatmapper.github.io/CatMapR/reference/uploadInputNodes.md)**:
  Upload edit-page rows to CatMapper’s `/uploadInputNodes` endpoint
  (write operation; API key required).
- **[`submitEditUpload()`](https://projectcatmapper.github.io/CatMapR/reference/submitEditUpload.md)**:
  Run the edit upload flow and automatically trigger waiting-USES
  contextual relationship refresh in the background.

### Legacy-compatible aliases

- **[`allDatasets()`](https://projectcatmapper.github.io/CatMapR/reference/allDatasets.md)**:
  Legacy-compatible alias for
  [`listDatasetMetadata()`](https://projectcatmapper.github.io/CatMapR/reference/listDatasetMetadata.md).
...
- **[`callAPI()`](https://projectcatmapper.github.io/CatMapR/reference/callAPI.md)**:
  Internal helper used by exported wrappers. This is not part of the
  preferred user-facing API.

### Metadata helper notes

- [`getUploadProperties()`](https://projectcatmapper.github.io/CatMapR/reference/getUploadProperties.md)
  targets the canonical production API and is intended for upload/edit
  introspection.
- [`getProperties()`](https://projectcatmapper.github.io/CatMapR/reference/getProperties.md)
  depends on deployments that expose
  `GET /metadata/properties/<database>`. During rollout, some
...
- **Returns metadata and API responses**, including dataset catalog
  fields (for example CMID, CMName, citations, relationships), category
  matches, translation/join outputs, and metadata/property tables.
- **Does not download dataset source files** managed outside CatMapper.
  User-owned raw datasets remain external inputs to your R workflow.
...
|------------------------|--------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `/:database/explore`   | Search and inspect entities/categories                                   | [`searchDatabase()`](https://projectcatmapper.github.io/CatMapR/reference/searchDatabase.md), [`CMIDinfo()`](https://projectcatmapper.github.io/CatMapR/reference/CMIDinfo.md), [`getDomains()`](https://projectcatmapper.github.io/CatMapR/reference/getDomains.md)                                       |
| `/:database/translate` | Translate labels and review proposed matches                             | [`translate()`](https://projectcatmapper.github.io/CatMapR/reference/translate.md)                                                                                                                                                                                                                         |
| `/:database/merge`     | Propose key mappings and join aligned tables                             | [`createLinkfile()`](https://projectcatmapper.github.io/CatMapR/reference/createLinkfile.md), [`joinDatasets()`](https://projectcatmapper.github.io/CatMapR/reference/joinDatasets.md)                                                                                                                     |
...
Write endpoints (for example uploads) require a valid API key from a
registered CatMapper account:

``` r
Sys.setenv(CATMAPR_API_KEY = "cmk_your_api_key")
```

CatMapR reads `CATMAPR_API_KEY` first, and falls back to
`CATMAPPER_API_KEY` when needed.

- How to get an API key:
  <https://catmapper.org/help/API.html#api-key-access>
- For write upload calls, CatMapper identifies the acting user from the
  API key on the server side.
- Server-side permissions determine whether that user can run the
  requested write action.
- CatMapR does not implement username/password login flows; it sends
  API-key-authenticated requests for write uploads.
...
Returned metadata keys can be stored-form values (for example
`Key == Region == Flagstaff`). Use
[`normalize_key()`](https://projectcatmapper.github.io/CatMapR/reference/normalize_key.md)
before reusing keys in upload payloads.
...
- `so = "standard"` (recommended): `Key` values must already be full
  expressions like `VARIABLE == VALUE` (and may include `&&`).
- `so = "simple"`: only supported with `ao = "add_uses"` and key values
  must be raw values only (for example `eth:yoruba`) with no `==`.

If `so = "simple"` receives preformatted keys (contains `==`), CatMapR
...
connect to other CMIDs. This refresh is fire-and-forget and is not
...
The merge-template endpoints used by CatMapperJS are available without
an API key. CatMapR exposes wrappers for the main read and generate
flows:
...
CatMapR relies on the following R packages: - `httr`: For making HTTP
requests to the CatMapper API. - `jsonlite`: For handling JSON
responses. - `tictoc`: For timing API calls.
